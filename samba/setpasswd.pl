...
if($item=~(/(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/))
    {
     ...
     {
      print "$login $access";
      system"mkdir /usr/sch/sites/home/$login/$access";
      system"mount -t union /usr/sch/sites/home/$login/$access /usr/sch/sites/home/$access";
     }
     if (not ($share=~m/none/))
     {
      print "$login $access";
      system"mkdir /usr/sch/sites/home/$login/$share";
      #system"mount -t union /usr/sch/sites/home/$login/$access /usr/sch/sites/home/$access";
     }
